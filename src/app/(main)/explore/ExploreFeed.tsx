...
export default function ExploreFeed() {
  const {
    ...
    // Keeps the shared ["post-feed", ...] prefix so the optimistic cache
    // surgery in components/posts/mutation.ts still matches this feed.
    queryKey: ["post-feed", "explore"],
    queryFn: ({ pageParam }) =>
      kyInstance
        .get(
          "/api/posts/explore",
      ...
      <div className="bg-card border-border flex flex-col items-center gap-3 rounded-2xl border p-10 text-center shadow-sm">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Follow a few people or post something, and recommendations will
            show up here.
          </p>
        </div>
      </div>
    ...
    return (
      <div className="bg-card border-border flex flex-col items-center gap-3 rounded-2xl border p-10 text-center shadow-sm">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            We couldn&apos;t load recommendations. Check your connection and try
            again.
          </p>
        </div>
